/**
 * Item matching algorithm — compares lost vs found reports.
 */
...
export function computeMatchScore(lost, found) {
  ...
  if (lost.category === found.category) score += 35;

  score += overlapScore(lost.itemName, found.itemName) * 25;
  ...
  if (lost.location && found.location) {
    score += overlapScore(lost.location, found.location) * 10;
  }
...
export function findPossibleMatches(items, threshold = 40) {
...
export function findMatchesForItem(item, allItems, threshold = 35) {
